...
import ReportFilterCh, {
  REPORT_ALL_DEVICES_GROUP_ID,
  REPORT_UNGROUPED_DEVICES_GROUP_ID,
} from './ReportFilterCh';
import ArrowBackIcon from '@mui/icons-material/ArrowBack';

const ReportFilter = ({
  children, handleSubmit, handleSchedule, showOnly, ignoreDevice, multiDevice,
  includeGroups, groupOnly, loading,
}) => {
  ...
  const selectionDisabled = groupOnly
    ? !groupIds.length
    : (!ignoreDevice && !deviceId && !deviceIds.length && !groupIds.length);
  const disabled = selectionDisabled || scheduleDisabled || loading;
  ...
  const getReportSelection = () => {
    if (!groupOnly) {
      return { selectedDeviceIds: deviceIds, selectedGroupIds: groupIds };
    }
    const allDevices = groupIds.includes(REPORT_ALL_DEVICES_GROUP_ID);
    const includeUngrouped = groupIds.includes(REPORT_UNGROUPED_DEVICES_GROUP_ID);
    const selectedDeviceIds = allDevices
      ? Object.keys(devices)
      : Object.values(devices)
        .filter((device) => includeUngrouped && !device.groupId)
        .map((device) => device.id);
    const selectedGroupIds = allDevices
      ? []
      : groupIds.filter((id) => (
        id !== REPORT_ALL_DEVICES_GROUP_ID
        && id !== REPORT_UNGROUPED_DEVICES_GROUP_ID
      ));
    return { selectedDeviceIds, selectedGroupIds };
  };
  
  const handleClick = (type) => {
    const { selectedDeviceIds, selectedGroupIds } = getReportSelection();
    if (type === 'schedule') {
      handleSchedule(selectedDeviceIds, selectedGroupIds, {
        description,
        calendarId,
        attributes: {},
      });
      ...
      handleSubmit({
        deviceId: groupOnly ? null : deviceId,
