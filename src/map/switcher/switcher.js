...
export class SwitcherControl {
  ...
  onSelectStyle(target) {
    ...
    this.onAfterSwitch(this.currentStyle);
  }

  selectStyle(styleId) {
    const target = Array.from(this.mapStyleContainer.children)
      .find((element) => element.dataset.id === styleId);
    if (target && this.currentStyle !== styleId) {
      this.onSelectStyle(target);
      return true;
    }
    return false;
