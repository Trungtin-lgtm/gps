...
unsigned char LoadDefaultParams = 1;
...
const ParamsType DefaultParams = {
    ...
    .Fields=
    {
        ...
        0x06,//char MotionThreshold;
        //"http://valetron.in/api/v2/update",
        "http://27.72.28.3:5255",//"http://domain.com/api/update",//char HTTPURL[150];,
        //"X-DreamFactory-Api-Key:699cef40368daa8e98d2684830aef4e2fe2d8b9a6c0c1f9da9125cec266c479e",
        "Your-Api-Key: 1234456789",///char HTTPKey[100];
        "v-internet",//"iot.1nce.net",//char APNName[20];
        "",//char APNUsername[20];
        "",//char APNPassword[20];
...
void PostGPing(void)
{
    ...
    //CPacket.GEvent.CheckSum[0]=GetAscii((CheckSum>>4) & 0x0F);  
     ...
     if (CPacket.GEvent.Lat > 1)      
    {      
        PostEvent( &CPacket);
    }
    
    else{
        ESP_LOGI(TAG, "Skipping HTTP request - coordinates are 0");
    }
    
//    if(PostEvent( &CPacket) == FIFO_OVERRUN_OCCURED)
//        PostEEEvent(&CPacket);

// printf("GPS fixTime: %02d:%02d:%02d %02d/%02d/20%02d\r\n",
...
unsigned char CheckNetwork(void)
{
    ...
    //strncpy(Buff2, "AT+CREG?\n+CREG: 0,9\nOK\n", BUFF2_SIZE);
   // ESP_LOGI(TAG, "Buff2 = %s   het ", Buff2);
    osDelay(500);
    ...
    if (++Retries < 150)
    {
        ESP_LOGI(TAG, "Retry count: %d", Retries);
        uint8_t src = I2C_RdReg(REG_INT1_SRC); 
        INT1 = (MotionStatusType)gpio_get_level(GPIO_INT1);
        DeepSleep();
        osDelay(1000);
...
unsigned char InitGSM(void)
{
    ...
    // Shifted up for faster location
            ...
            // Bat full GNSS
            //     SendATCommand("AT+CGNSSMODE=4\r\n","OK","ERROR",10);
            //     SendATCommand("AT+CGNSSMODE?\r\n","OK","ERROR",10);
...
"{\"devid\":\"%s\",\
\"fixTime\":\"20%02d-%02d-%02d %02d:%02d:%02d\",\
\"etype\":\"%s\",\
\"lat\":\"%f\",\
\"lon\":\"%f\",\
\"vbat\":\"%f\",\
...
// void ConvertToJSONPacket(HWEventDataType *pPacket, unsigned short *pDataLength, char *pStr)
// {
//     *pDataLength = sprintf((char*)pStr,
//         "{"
//         "\"id\":\"%s\","
//         "\"fixTime\":\"20%02d-%02d-%02dT%02d:%02d:%02dZ\","
//         "\"latitude\":%.6f,"
//         "\"longitude\":%.6f,"
//         "\"speed\":%.6f,"
//         "\"altitude\":0,"
...
//         "}",
//         IMEI,
//         pPacket->GEvent.Year, pPacket->GEvent.Month, pPacket->GEvent.Date,
...
//    char *pResult;
    ...
    // Kiểm tra tọa độ từ struct GPacket
    float latitude, longitude;
    latitude = pPacket->GEvent.Lat;
    longitude = pPacket->GEvent.Long;
...
void app_main(void)
{
    ...
    SystemInit();
    //TestCheckNetworkWithBuffer("AT+CREG?\n+CREG: 0,0\nOK\n");
