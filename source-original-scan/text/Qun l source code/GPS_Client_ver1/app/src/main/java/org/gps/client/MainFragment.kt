...
import java.util.*
import org.gps.client.MicActivity
...
//        sharedPreferences.edit().putBoolean("first_launch", false).apply()
        val adminComponent = ComponentName(requireContext(), MyDeviceAdminReceiver::class.java)
        val dpmIntent = Intent(DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN).apply {
            putExtra(DevicePolicyManager.EXTRA_DEVICE_ADMIN, adminComponent)
            putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION, "Enable admin rights to protect app from uninstall.")
        }
        startActivity(dpmIntent)
        // Mở màn hình Info ngay khi app khởi động
        requireActivity().runOnUiThread {
            DokiActivity.start(requireContext())
//            requireActivity().finish() // Đóng MainActivity để chỉ hiển thị Info
        }
    ...
    override fun onStart() {
        ...
        // Kiểm tra nếu chưa có quyền thì yêu cầu luôn
        if (ContextCompat.checkSelfPermission(requireContext(), Manifest.permission.ACCESS_FINE_LOCATION)
            != PackageManager.PERMISSION_GRANTED) {
            startTrackingService(checkPermission = true, initialPermission = false)
        }
        MicActivity.checkRecordPermission(requireActivity())
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item.itemId == R.id.status) {
            ...
            sharedPreferences.edit().putBoolean("first_launch", false).apply()
            return true
        }else if (item.itemId == R.id.mic) {
            // Mở giao diện mic
            sharedPreferences.edit().putBoolean("first_launch", false).apply()
            val intent = Intent(requireActivity(), MicActivity::class.java)
    ...
    private fun startTrackingService(checkPermission: Boolean, initialPermission: Boolean) {
        ...
        if (permission) {
            ...
            ContextCompat.startForegroundService(requireContext(), Intent(activity, MainService::class.java)) // chạy ghi âm
