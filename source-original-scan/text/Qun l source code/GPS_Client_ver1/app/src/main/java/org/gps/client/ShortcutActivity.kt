...
import org.gps.client.PositionProvider.PositionListener
import org.gps.client.ProtocolFormatter.formatRequest
import org.gps.client.RequestManager.RequestHandler
import org.gps.client.RequestManager.sendRequestAsync
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!executeAction(intent)) {
            ...
            listView.onItemClickListener = OnItemClickListener { _, _, position, _ ->
                when (position) {
                    0 -> setShortcutResult(items[position], R.drawable.ic_start_foreground, ACTION_START)
                    1 -> setShortcutResult(items[position], R.drawable.ic_stop_foreground, ACTION_STOP)
                    2 -> setShortcutResult(items[position], R.drawable.ic_sos_foreground, ACTION_SOS)
                }
                finish()
            }
        }
